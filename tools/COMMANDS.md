...
authurlloc=$(cf service-key ACC_CIS_CEN ACC_CIS_CEN_Key | tail -n +3 | jq .uaa.url | tr -ds '"' '')
clientidloc=$(cf service-key ACC_CIS_CEN ACC_CIS_CEN_Key | tail -n +3 | jq .uaa.clientid | tr -ds '"' '')
clientsecretloc=$(cf service-key ACC_CIS_CEN ACC_CIS_CEN_Key | tail -n +3 | jq .uaa.clientsecret | tr -ds '"' '')
accountsurlloc=$(cf service-key ACC_CIS_CEN ACC_CIS_CEN_Key | tail -n +3 | jq .endpoints.accounts_service_url | tr -ds '"' '')
provisioningurlloc=$(cf service-key ACC_CIS_CEN ACC_CIS_CEN_Key | tail -n +3 | jq .endpoints.provisioning_service_url | tr -ds '"' '')
